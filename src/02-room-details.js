...
const getConnectedRoomNamesById = (rooms, id) => {
  ...
  return room.connectsTo.map((connectedRoomId) => {
    const room = rooms.find((room) => room.roomId === connectedRoomId);
    if (!room) {
      throw `Room with ID of '${connectedRoomId}' could not be found.`
    }
